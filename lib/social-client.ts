...
export async function banUserAsAdmin(userId: string, banned: boolean) {
  const auth = await requireUserWithProfile()
  if ('error' in auth && auth.error) return { error: auth.error }
  const { supabase, profile } = auth as {
    supabase: NonNullable<ReturnType<typeof createClient>>
...
export async function getFeaturedReviews() {
  const supabase = createClient()
  if (!supabase) return { reviews: [] as Review[] }
...
export async function sendTasteRecommendation() {
  const auth = await requireUser()
  ...
  if (!favorite) return { skipped: true }

  const label = favorite.item_artist
    ? `${favorite.item_name} — ${favorite.item_artist}`
    : favorite.item_name
  ...
  await supabase.from('notifications').insert({
    ...
    message: `Explorez plus de musique proche de ${label} dans les tendances.`,
    link: '/charts',
  })
