...
export async function startConversation(otherUserId: string): Promise<{
  conversationId?: string
  error?: string
}> {
  const supabase = await createClient()
...
export async function searchUsersForMessaging(query: string): Promise<{
  ...
  const { data: users, error } = await supabase
    .from('profiles')
    .select('id, username, display_name, avatar_url, bio, website, is_admin, is_banned, theme_preference, notification_email, notification_push, created_at, updated_at')
    .neq('id', user.id)
