...
export async function getMyCollections(): Promise<{ collections?: Collection[]; error?: string }> {
  ...
  const { supabase, user } = auth as { supabase: NonNullable<ReturnType<typeof createClient>>; user: { id: string } }
...
export async function updateList(
  ...
  if (input.is_public != null) payload.is_public = input.is_public
...
export async function removeItemFromList(listId: string, itemId: string) {
  ...
  const newCount = Math.max(0, (list.item_count ?? 1) - 1)
  await supabase
    .from('lists')
